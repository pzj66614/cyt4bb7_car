            ...
            if(motor_value.receive_data_buffer[0] == 0xA5)                                  // 判断帧头是否正确
            {
                ...
                if(motor_value.sum_check_data == motor_value.receive_data_buffer[6])        // 校验数据准确性
                {
                    ...
                    if(motor_value.receive_data_buffer[1] == 0x02)                          // 判断是否正确接收到 速度输出 功能字
                    {
                        ...
                        // 修正：右侧电机方向取反（根据实际机械安装调整）
                        motor_value.receive_right_speed_data = -motor_value.receive_right_speed_data;
                    }
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     无刷驱动 设置电机转速
// 参数说明     left_speed      左侧电机转速
// 参数说明     right_speed     右侧电机转速
// 返回参数     void
...
void small_driver_set_speed(int16 left_speed, int16 right_speed)
{
    ...
    motor_value.send_data_buffer[1] = 0X04;                                         // 配置功能字

    motor_value.send_data_buffer[2] = (uint8)((left_speed & 0xFF00) >> 8);          // 拆分 左侧转速 的高八位

    motor_value.send_data_buffer[3] = (uint8)(left_speed & 0x00FF);                 // 拆分 左侧转速 的低八位

    motor_value.send_data_buffer[4] = (uint8)((right_speed & 0xFF00) >> 8);         // 拆分 右侧转速 的高八位

    motor_value.send_data_buffer[5] = (uint8)(right_speed & 0x00FF);                // 拆分 右侧转速 的低八位
    ...
    uart_write_buffer(SMALL_DRIVER_UART, motor_value.send_data_buffer, 7);                     // 发送设置转速的 字节包 数据
}
...
void small_driver_uart_init(void)
{
    ...
    small_driver_set_speed(0, 0);                                                               // 设置0转速
